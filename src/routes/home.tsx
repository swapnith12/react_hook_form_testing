...
export default function Home() {
  ...
  if (isLoading) {
   ...
   if (error) {
    return <p className="text-red-500">Error: {error.message});
    </p>;
  }

  if(data){
  return (
    <div className="p-4 w-screen h-screen justify-center">
      {data.length === 0 ? (
        <p className="text-white">Loading jobs...</p>
      ) : (
        <ul className="space-x-8 space-y-2 flex flex-row flex-wrap">
          {data.map((job: { jobId: any; role?: string | undefined; salary?: string | undefined; description?: string | undefined; experience?: string | undefined; }) => (
